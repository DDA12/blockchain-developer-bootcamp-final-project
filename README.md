...
- ### **Current implementation**
  ...
  - ***Decentralized Off-Chain storage:***  
    IPFS and Filecoin are both used as the Decentralized Off-Chain storage. It is currently functional. The creation of an NFT generates a COA (metadata in a JSON Web Token Signed format) and media that are stored on IPFS and Filecoin. Only the CID of the COA is stored on-chain. The creation of a Portfolio also stores a media off-chain. The current dApp runs an IPFS client node, connects to a remote IPFS node and uses [Web3.storage](https://web3.storage) to deliver the functionality. It uses the following libraries: [ipfs-core](https://github.com/ipfs/js-ipfs/tree/master/packages/ipfs-core), [ipfs-http-client](https://github.com/ipfs/js-ipfs/tree/master/packages/ipfs-http-client) and [web3.storage JS client](https://github.com/web3-storage/web3.storage).
