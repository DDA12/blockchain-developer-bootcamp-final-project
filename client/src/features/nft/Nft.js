...
import {ipfsGatewayUrl, chainList} from "chain.js"
...
function updateNftHtml() {
    ...
    if (stateWallet.currentAccount!= null && ethersUtils.getAddress(stateWallet.currentAccount) == ethersUtils.getAddress(stateRegistry.portfolioSelected.owner)) {
        ...
        case statusRegistry.PORTFOLIOSELECTED:
            ...
            $(`#${CERATENFTBUTTON}`).off('click').on('click', ()=>{
                ...
                if (!stateWallet.currentAccount || !stateWallet.currentChainId) {
                    removeAllAlerts()
                    renderAlert(`AlertCreateNft`+v4(), 'Create NFT:', 'Please "Sign in" to be able to create an NFT.', 'warning', 7000)
                    return
                }       
                if (ethersUtils.getAddress(stateWallet.currentAccount) != ethersUtils.getAddress(portfolioSelected.owner)) {
                    removeAllAlerts()
                    renderAlert(`AlertCreateNft`+v4(), 'Create NFT:', 'You are not the Owner of this Portfolio. You can\'t create an NFT.', 'warning', 7000)
                    return
                }
                if (parseInt(stateWallet.currentChainId) != parseInt(portfolioSelected.chainId)) {
                    ...
                    renderAlert(`AlertCreateNft`+v4(), 'Create NFT:', `Please change the chain/network in MetaMask to "${name}" to create an NFT in this Portfolio.`, 'warning', 7000)
                    return
                }        
...
async function  renderNftModal(nft, portfolioSelected, tokenId, localUri) {
    ...
    const modalInstance = Modal.getOrCreateInstance(document.getElementById(SHOWNFTMODALID))
    const $showNftModal = $(`#${SHOWNFTMODALID}`)
    $showNftModal.on('hidden.bs.modal', function () {
        modalInstance.hide()
        $showNftModal.remove()
      })
...
async function renderCards(containerId, listNfts, walletUser) {
    ...
    for (let tokenId in listNfts) {
        ...
        if (card.length == 0) {
            ...
            $(`#${jti}`).hide().slideDown(750).css('opacity', 0).animate({opacity: 1}, {queue: false, duration: 750})
            addSpinnerHTML(`${COLLECTIONIMAGEID}${jti}`, `${SPINNERID}${jti}`, true)
            addSpinnerHTML(`${IMAGEJWSID}${jti}`, `${JWSSPINNERID}${jti}`, true, 'text-warning')
            DS.get(nftMetaData.image)
                .then((file)=> {return URL.createObjectURL(file)})
                .then((localUri) => {
                ...
                .then(() => {
                    return verifyJws(listNfts[tokenId].jws).then(() => {
        ...
        if (walletUser.currentAccount!= null && ethersUtils.getAddress(walletUser.currentAccount) == ethersUtils.getAddress(nft.owner)) {
