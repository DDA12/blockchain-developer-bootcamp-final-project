...
import { portFolioSelected, loadContractsDeployed, status as statusRegistry, createPortfolio, ready, nftAdded} from './registrySlice.js';
...
import {chainList} from "../../common/chain.js";
...
export function initPortfolio() {
    ...
    $(`#${CERATEPORTFOLIOBUTTON}`).off('click').on('click', ()=>{
        ...
        if (state.status != statusWallet.SIGNEDIN &&  state.status != statusWallet.CHAINIDCHANGED) {
            removeAllAlerts();
            renderAlert(`AlertCreatePortfolio`+uuid.v4(), 'Create Portoflio:', 'Please "Sign in" to create a portfolio.', 'warning', 5000);
            return;
        }
        if (chainList[parseInt(state.currentChainId)] == undefined) {
            removeAllAlerts();
            renderAlert(`AlertCreatePortfolio`+uuid.v4(), 'Create Portoflio:', `Unsupported Chain/Network: "${state.currentChainId}". Please change the chain/network in MetaMask to create a Portfolio.`, 'warning', 7000);
            return;
        }
...
function renderCards(containerId, listContracts, walletUser) {
    for (const chainKey in listContracts) {
        ...
        for (let address in chainListContracts) {
            ...
            if (walletUser.currentAccount!= null && ethers.utils.getAddress(walletUser.currentAccount) == ethers.utils.getAddress(contract.owner)) {
