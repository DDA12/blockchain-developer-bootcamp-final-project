...
function addReadingContract(chainKey, portfolioAddress, etherProvider) {
  const readingContract = new ethers.Contract(portfolioAddress, etherProvider.nftContractBuild.abi, etherProvider.provider);
...
export const createPortfolio = RTK.createAsyncThunk("registry/createPortfolio", async (param, thunkAPI) => {
  const cid = await addFileIpfs(param.url).catch((e) => {console.log(e); return e});
    ...
    const writeContract = new ethers.Contract(etherProvider.portfolioRegistryContractAddress, etherProvider.portfolioRegistryContractBuild.abi, signer);
    ...
    const iface = new ethers.utils.Interface(etherProvider.portfolioRegistryContractBuild.abi );
...
export const loadContractsDeployed = RTK.createAsyncThunk("registry/loadContractsDeployed", async (arg, thunkAPI) => {
...
export const registrySlice = RTK.createSlice({
    name: 'registry',
    ...
    reducers: {
        ...
        createTransactionSubmitted:(state, action) =>  {
    ...
    extraReducers(builder) {
      builder
      .addCase(loadContractsDeployed.pending, (state, action) => {
        state.status = status.LOADING;
      })
      .addCase(loadContractsDeployed.fulfilled, (state, action) => {
        state.status = status.READY;
        state.error = null;
    })
