...
import { walletDetect, status, walletConnect, walletChainIdChanged, disconnected, walletAccountChanged, walletSignIn, getWalletProvider } from './walletSlice.js'
...
import {chainList, ipfsGatewayUrl} from "../../common/chain.js";
...
const walletNavBarChain = $('#'+walletNavBarChainId);
$("#navBarAccount").append(walletNavBarAccountHTML());
const walletNavBarAccount = $('#'+walletNavBarAccountId);
$("#navBarBalance").append(walletNavBarBalanceHTML());
const walletNavBarBalance = $('#'+walletNavBarBalanceId);
hideAccountInfo();
...
function renderHelpModal() {
    ...
    const modalInstance = bootstrap.Modal.getOrCreateInstance(document.getElementById(HELPMODALID));
...
function  renderProfileModal() {
    ...
    const modalInstance = bootstrap.Modal.getOrCreateInstance(document.getElementById(CREATEPROFILEMODALID));
    const $createCollectionModal = $(`#${CREATEPROFILEMODALID}`);
    $createCollectionModal.on('hidden.bs.modal', function (event) {
        modalInstance.hide();
      ...
      $createCollectionModal.on('shown.bs.modal', () => {
        ...
        return new Promise(async () => {
            const doc = await resolveDidDocument(user.currentDid);
...
function renderModalMetaMaskMissing() {
    ...
    const modalInstance = bootstrap.Modal.getOrCreateInstance(document.getElementById(WALLETMODALID));
